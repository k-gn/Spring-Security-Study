...
import com.example.springsecuritypractice.jwt.JwtAuthorizationFilter;
import com.example.springsecuritypractice.jwt.JwtProperties;
...
public class SpringSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        // stateless (토큰 방식은 세션이 필요없음)
        http.sessionManagement()
                .sessionCreationPolicy(SessionCreationPolicy.STATELESS); // 세션 정책을 사용하지 않음
        // jwt filter
...
//        http.rememberMe(); // 자동 로그인 (remember-me 쿠키 발행), rememberMeAuthenticationFilter 활성화
        ...
        http.logout() // 로그아웃 설정
                ...
                .deleteCookies(JwtProperties.COOKIE_NAME); // 로그 아웃시 해당 쿠키 제거
    }
